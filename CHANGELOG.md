...
- **`.parquet` input** — CLI scatter (and all future subcommands via `DataTable::parse`) can read `.parquet` files directly, or detect parquet piped via stdin by magic bytes. Uses the parquet row API; no Arrow dependency. Gated behind `--features parquet`.
