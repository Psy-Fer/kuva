...
fn test_forest_basic() {
    ...
    assert!(
        svg.contains("Study A"),
        "SVG should contain row label Study A"
    );
    assert!(
        svg.contains("Study B"),
        "SVG should contain row label Study B"
    );
    assert!(
        svg.contains("Study C"),
        "SVG should contain row label Study C"
    );
}
...
fn test_forest_null_line() {
    ...
    let plots = vec![Plot::Forest(forest)];
    let layout = Layout::auto_from_plots(&plots).with_title("Forest Plot Null Line");
    ...
    assert!(
        ...
        "SVG should contain a dashed null reference line"
    );
}
...
fn test_forest_weighted() {
    ...
    let plots = vec![Plot::Forest(forest)];
    let layout = Layout::auto_from_plots(&plots).with_title("Forest Plot Weighted");
    ...
    assert!(
        svg.contains("<rect"),
        "SVG should contain rect elements for weighted markers"
    );
}
...
fn test_forest_custom_color() {
    ...
    let plots = vec![Plot::Forest(forest)];
    let layout = Layout::auto_from_plots(&plots).with_title("Forest Plot Custom Color");
...
fn test_forest_single_row() {
    let forest = ForestPlot::new().with_row("Only Study", 0.30, -0.10, 0.70);

    let plots = vec![Plot::Forest(forest)];
    let layout = Layout::auto_from_plots(&plots).with_title("Forest Plot Single Row");
    ...
    assert!(
        ...
        "SVG should contain the single row label"
    );
}
...
fn test_forest_wide_ci() {
    ...
    assert!(
        svg.contains("<rect"),
        "even tiny-relative CIs should have markers"
    );
}
...
fn test_forest_estimate_at_ci_edge() {
    ...
    assert!(
        rect_count >= 4,
        "expected at least 4 rects (bg + 3 markers), got {rect_count}"
    );
}
...
fn test_forest_no_null_line() {
    ...
    assert!(
        !svg.contains("stroke-dasharray"),
        "no dashed line when null line is disabled"
    );
}
...
fn test_forest_extreme_weights() {
    ...
    assert!(
        rect_count >= 4,
        "expected at least 4 rects, got {rect_count}"
    );
}
...
fn test_forest_long_labels() {
    let forest = ForestPlot::new()
        .with_row(
            "Randomized Controlled Trial of Extended Duration Therapy (Phase III)",
            0.3,
            0.1,
            0.5,
        )
...
fn test_forest_all_positive() {
    ...
    assert!(
        svg.contains("stroke-dasharray"),
        "null line at 0 should be visible even when all CIs are positive"
    );
}
...
fn test_forest_with_caps() {
    ...
    assert!(
        line_count >= 8,
        "expected at least 8 lines with caps, got {line_count}"
    );
}
...
fn test_forest_weighted_colored_rows() {
    ...
    let plots = vec![Plot::Forest(forest)];
    let layout = Layout::auto_from_plots(&plots).with_title("Forest Plot Weighted + Colored");
    ...
    assert!(
        svg.contains("#4682b4"),
        "SVG should contain #4682b4 (steelblue)"
    );
    assert!(
        svg.contains("#ffa500"),
        "SVG should contain #ffa500 (orange)"
    );
    assert!(
        svg.contains("#800080"),
        "SVG should contain #800080 (purple)"
    );
    // Weighted rows produce rects of different sizes; there should be at least 3 rects
    let rect_count = svg.matches("<rect").count();
    assert!(
        rect_count >= 3,
        "expected at least 3 rects for weighted markers, got {rect_count}"
    );
}
