...
fn make_scatter_svg(interactive: bool) -> String {
    let mut layout = Layout::auto_from_plots(&[Plot::Scatter(ScatterPlot::new().with_data(vec![
        (1.0_f64, 2.0),
        (3.0, 4.0),
        (5.0, 1.0),
    ]))]);
...
fn test_interactive_off_by_default() {
    ...
    assert!(
        !svg.contains("<script"),
        "non-interactive SVG must not have <script"
    );
    assert!(
        !svg.contains("<foreignObject"),
        "non-interactive SVG must not have <foreignObject"
    );
}
...
fn test_interactive_root_attrs() {
    ...
    assert!(
        svg.contains("data-plot-bottom="),
        "missing data-plot-bottom"
    );
}
...
fn test_interactive_scatter_data_attrs() {
    ...
    assert!(
        svg.contains("data-y="),
        "expected data-y= on scatter point groups"
    );
}
...
fn test_interactive_no_circle_batch() {
    ...
    assert!(
        svg.contains("data-x="),
        "scatter points should be individually wrapped in interactive mode"
    );
    // Non-interactive mode has no data-x
    let svg_off = make_scatter_svg(false);
    assert!(
        !svg_off.contains("data-x="),
        "non-interactive scatter must not have data-x"
    );
}
...
fn test_interactive_volcano_point_logfc() {
    let svg = make_volcano_svg(true);
    assert!(
        svg.contains("data-logfc="),
        "expected data-logfc= on volcano point groups"
    );
}
...
fn test_interactive_legend_data_group() {
    ...
    assert!(
        svg.contains("data-group="),
        "expected data-group= on legend entry groups"
    );
}
...
fn test_interactive_script_present() {
    ...
    assert!(
        svg.contains("<script"),
        "expected <script in interactive SVG"
    );
...
fn test_interactive_foreignobject() {
    ...
    assert!(
        svg.contains("<foreignObject"),
        "expected <foreignObject in interactive SVG"
    );
    assert!(
        svg.contains("kuva-search"),
        "expected search input in foreignObject"
    );
}
...
fn test_interactive_css_pinned() {
    ...
    assert!(
        svg.contains(".pinned"),
        "expected .pinned CSS rule in interactive SVG"
    );
}
...
fn test_noninteractive_unchanged() {
    ...
    assert!(
        !svg.contains("data-xmin"),
        "non-interactive must not have axis metadata"
    );
    assert!(
        !svg.contains(".pinned"),
        "non-interactive must not have pinned CSS"
    );
}
...
fn test_interactive_figure_propagates() {
    ...
    assert!(
        svg.contains("data-x="),
        "figure panels should have data-x on scatter points when interactive"
    );
}
